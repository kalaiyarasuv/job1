...
if (empty($full_name) || !preg_match("/^[a-zA-Z][a-zA-Z ]*$/", $full_name)) {
    echo "<script>alert('Enter valid username.'); window.location.href = 'emreg.html';</script>";
    exit();
}
...
if (empty($password)) {
    echo "<script>alert('Password cannot be empty.'); window.location.href = 'emreg.html';</script>";
    exit();
}
if (strpos($password, ' ') !== false) {
    echo "<script>alert('Password must not contain spaces.'); window.location.href = 'emreg.html';</script>";
    exit();
}

// Password confirmation check
if ($password !== $confirm_password) {
    echo "<script>alert('Passwords do not match!'); window.location.href = 'emreg.html';</script>";
    exit();
}

// Validate email format
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo "<script>alert('Invalid email format!'); window.location.href = 'emreg.html';</script>";
    exit();
}

// Check if the email already exists
$email_check_query = "SELECT * FROM emlog WHERE email = ?";
...
if ($stmt->execute()) {
    ...
    echo "<script>alert('Error: Could not create account. Please try again later.'); window.location.href = 'emreg.html';</script>";
}
